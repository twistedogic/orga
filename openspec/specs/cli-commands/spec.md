...
The CLI SHALL provide `orga ticket list` to output all tickets currently assigned to the configured agent. Output SHALL include ticket ID, title, list name, and URL. With `--json`, output SHALL be a JSON array of ticket objects.

#### Scenario: Tickets assigned to agent
- **WHEN** the agent has one or more assigned tickets
- **THEN** each ticket is printed with its ID, title, current list, and URL

#### Scenario: No assigned tickets
- **WHEN** the agent has no assigned tickets
- **THEN** the command exits with code 0 and prints nothing (or empty JSON array with `--json`)

#### Scenario: JSON output
- **WHEN** `--json` flag is passed
- **THEN** output is a valid JSON array of ticket objects with fields: `id`, `title`, `list`, `url`, `description`

### Requirement: Ticket show command
The CLI SHALL provide `orga ticket show <id>` to output the full context of a ticket: title, description, current list, assignees, checklist items, and all comments in chronological order.

#### Scenario: Ticket exists
- **WHEN** a valid ticket ID is provided
- **THEN** full ticket context is printed including all comments and checklist items

#### Scenario: JSON output
- **WHEN** `--json` flag is passed
- **THEN** output is a JSON object with fields: `id`, `title`, `description`, `list`, `assignees`, `checklists`, `comments`
...
The CLI SHALL provide `orga ticket comment <id> <text>` to post a comment on a ticket as the configured agent.

#### Scenario: Comment posted successfully
- **WHEN** a valid ticket ID and non-empty comment text are provided
- **THEN** the comment is posted and a success message is printed
