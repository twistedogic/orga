# artifact-store-sync Specification

## Purpose
Defines the sync behaviour of `GitArtifactStore` for read operations: fetch-before-read when a remote is configured, with retry and graceful fallback to local state.
