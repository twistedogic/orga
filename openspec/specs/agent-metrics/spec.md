...
|-----------|----------------|
| `process_ticket` returned `Ok(TicketProcessingOutcome::Success)` | `success` |
| `process_ticket` returned `Err(_)` | `error` |
| `process_ticket` returned `Ok(TicketProcessingOutcome::Skipped)` | `skipped` |
| `process_ticket` returned `Ok(TicketProcessingOutcome::CapReached)` | `cap_reached` |

The histogram buckets SHALL cover 0.1s to 300s.

#### Scenario: Successful ticket observed
- **WHEN** `process_ticket` returns `Ok(TicketProcessingOutcome::Success)` (e.g., the LLM cycle ended via the `done` tool)
...
- **WHEN** `process_ticket` returns `Ok(TicketProcessingOutcome::CapReached)` (the LLM cycle hit the action cap without `done` or `skip`)
