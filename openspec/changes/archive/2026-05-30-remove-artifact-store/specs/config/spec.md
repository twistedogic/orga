...
### Requirement: Config schema (artifact sections)
**Reason**: Artifact store has been removed. The `[artifact]` and `[artifact.git]` config sections no longer exist. The `max_artifact_inline_bytes` field on `[llm]` is also removed.
**Migration**: Use `[workspace]` for per-ticket file storage (`read_file`, `write_file`, `list_files` agent tools).
...
#### Scenario: Config with artifact section fails validation
- **WHEN** a config file includes an `[artifact]` section
- **THEN** the CLI exits with an error: "[artifact] section is no longer supported; use [workspace] for per-ticket file storage"
...
#### Scenario: Config with workspace section
- **WHEN** a config file includes a valid `[workspace]` section
- **THEN** `build_artifact_store` succeeds and returns a `GitArtifactStore`

#### Scenario: Config without workspace section
- **WHEN** the config file does not include `[workspace]`
- **THEN** workspace tools are unavailable; all other commands are unaffected
